...
class DualDefense(AbstractDefense):
    """
    ...
    def run_defense(self, observation: Dict[str, Any], page) -> Dict[str, Any]:
        ...
        self.log.info("Getting security policy")
        security_policy_start = time.time()
        security_policy = self.llm.get_security_policy(user_query, current_url)
        security_policy_end = time.time()
        self.log.info(f"Security policy returned in {security_policy_end - security_policy_start:.2f} seconds")
        ...
        relevance_labels = self.llm.get_html_relevance(redacted_html, user_query)
        relevance_end = time.time()
        self.log.info(f"Relevance labels returned in {relevance_end - relevance_start:.2f} seconds")
        annotated_dom.apply_relevance_labels(relevance_labels)

        allowed_elements_start = time.time()
        allowed_elements = self.llm.filter_by_security_policy_strict(annotated_dom, security_policy)
        print('relevance_labels:', relevance_labels)
        ...
        return {
            ...
            "relevance_labels": relevance_labels.model_dump(),
