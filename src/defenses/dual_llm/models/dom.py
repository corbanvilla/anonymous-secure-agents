...
from .policy import ContentOwners, DomMetadata, RelevanceLabels
...
class AnnotatedElement(BaseModel):
    ...
    relevance: Optional[str] = Field(
        None,
        description="Relevance label for this node",
    )
...
class AnnotatedDom(BaseModel):
    ...
    def apply_relevance_labels(self, relevance_labels: RelevanceLabels) -> List[int]:
        """
        Apply relevance labels to AnnotatedDom elements based on RelevanceLabels.
        """

        missing_bids: list[int] = []

        def get_relevance(bid: int) -> Optional[str]:
            """Get relevance label for a given bid."""
            if bid in relevance_labels.high:
                return "high"
            elif bid in relevance_labels.medium:
                return "medium"
            elif bid in relevance_labels.low:
                return "low"
            return None

        def recurse(nodes: List[AnnotatedElement], parent_relevance: Optional[str] = None):
            """Recursively apply relevance labels to nodes."""
            for node in nodes:
                node.relevance = get_relevance(node.bid)
                if not node.relevance:
                    node.relevance = parent_relevance
                    if parent_relevance:
                        missing_bids.append(node.bid)
                        logger.warning(
                            f"Missing relevance for bid {node.bid}, inheriting from parent: {parent_relevance}"
                        )
                    else:
                        logger.warning(f"No relevance found for bid {node.bid} and no parent relevance available")

                if node.children:
                    recurse(node.children, node.relevance)
