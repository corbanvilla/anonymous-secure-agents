...
'developer' - Site developer's content (account pages, documentation, navigation, sidebars, footers, etc)
'user' - User generated content (anything editable or manipulatable by site users - reviews, comments, forum posts, social media posts, usernames, etc.)
'third-party' - Other content (advertisements, sponsored links, affiliate content, products, etc.)
Output a subset of these levels to be allowed.

DECISION GUIDELINES
Integrity Selection:
Developer content is always included.
...
Product information, titles, descriptions, prices, etc.
...
→ Integrity: ['developer', 'third-party']
...
class PolicyAgent:
    ...
    # @record_timing()
    def get_security_policy(self, user_query: str, url: str, site_description: str) -> SecurityPolicy:
        payload = {
            "URL": url,
    ...
    def filter_by_security_policy_strict(
        ...
        def evaluate_element(element: AnnotatedElement) -> tuple[bool, str]:
            ...
            if element.skip_metadata:
                return False, "Skipped metadata, element not allowed"
