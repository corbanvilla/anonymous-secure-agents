...
from .enums import CapabilityLabel, ElementContext, ElementPurpose, IntegrityLabel
...
Owner labels:
 ...
 - user: User-generated content (comments, posts, uploaded media, user profiles, products, product descriptions, etc)
...
Your task is to consider the user's task and the provided abstracted HTML, then determine what capabilities the last element shown has.

Consider the principle of least privilege: the element should have the minimum capabilities necessary to complete the user's described task.

Capability labels:
 - viewable: The element is visible to the user
 - clickable: The element is clickable
 - typable: The element is typable

Capability labels are from least to most privileged. 
- If an element is typeable, it will automatically be viewable and clickable.
- If an element is clickable, it will automatically be viewable.
- If an element is viewable, it will only be viewable.

For instance, if the user's task is to research a topic on Reddit, user content may be viewable or potentially clickable (depending on if the agent is already viewing the post), but probably not typable. If the user needs to write a comment on a post, or interact with a product review, it makes sense to make the element typeable too. However, you should be most sparing with the typeable capability, as it provides the most privilege.

OUTPUT FORMAT
{
"capability": "viewable|clickable|typable"
}
"""
...
class AnnotatedElement(BaseModel):
    ...
    @computed_field
    def capability_map(self) -> Dict[int, Literal["viewable", "clickable", "typable"]]:
        """
    ...
    def as_html_tree_top_full(self) -> str:
        ...
        def _format_bracket(e: "AnnotatedElement") -> str:
            ...
            if e.metadata is not None:
                ...
                parts.append(f"context={e.metadata.context.value}")
                parts.append(f"purpose={e.metadata.purpose.value}")
    ...
    def as_direct_lineage_relevance_tree_top(self) -> str:
        ...
        attributes = {
            ...
            "context": element.metadata.context.value,
            "purpose": element.metadata.purpose.value,
        }
        while element is not None:
    ...
    def populate_metadata(
                ...
                try:
                    ...
                    if element.is_root:
                        element.metadata = AnnotatedElementMetadata(
                            ...
                            context=ElementContext.MAIN,
                            purpose=ElementPurpose.STRUCTURE,
                        )
                        return
                    ...
                    async with semaphore:
                        ...
                        # TODO - cache requests at the openai level
                        # TODO - test whether we should include metadata in the HTML output (for parents)
                        # TODO - test whether we can enable all capabilities for developer content
                        # TODO - optimize to stop requesting metadata for elements that are not allowed
                        ...
                        if element.metadata.owner == IntegrityLabel.DEVELOPER:
                            element.capabilities = CapabilityLabel.TYPABLE
                            return
                        ...
                        resp = await client.beta.chat.completions.with_raw_response.parse(
                            messages=[
                                ...
                                {"role": "user", "content": f"HTML: {element.as_direct_lineage_html_tree_top}"},
                            ],
        ...
        start = time.time()
        self._run_async_blocking(_run_all())
        end = time.time()
