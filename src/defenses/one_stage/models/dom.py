...
from .policy import BidSummary, ContentOwners, DomMetadata
...
class AnnotatedElement(BaseModel):
    ...
    name: Optional[str] = Field(None, description="The HTML tag name, e.g. 'meta', 'title', etc.", alias="tag_name")
    ...
    # injected metadata
    context: Optional[str] = Field(None, description="Where this node lives or applies")
    purpose: Optional[str] = Field(None, description="The intention or role of this node")
    metadata_group: Optional[int] = Field(None, description="The group ID of the metadata element this node belongs to")
    summary: Optional[str] = Field(None, description="A human‐readable summary of this node")
...
class AnnotatedDom(BaseModel):
    ...
    def fill_missing_bids(cls, tree: dict, parent_bid: int = -1) -> dict:
        ...
        for child in tree.get("children", []):
            logger.debug(f"Filling missing bid for child: {child} with parent bid: {parent_bid}")
        ...
        def recurse(nodes: List[AnnotatedElement]):
            for node in nodes:
                ...
                if meta:
                    ...
                    logger.debug("No metadata found for bid: %s", node.bid)
                if node.children:
    ...
    def apply_summary(self, summaries: BidSummary) -> list:
        """
        Recursively apply summary to nodes by bid using the provided BidSummary.
        Remove any node that does not have an associated summary.
        Returns a list of bids that were defined in multiple summary entries.
        """
        mapping = dict(zip(summaries.bids, summaries.summaries))
        multiple_definitions = []
        seen_bids = {}
        ...
        def recurse(nodes: List[AnnotatedElement]) -> List[AnnotatedElement]:
            filtered_nodes = []
            for node in nodes:
                if node.bid in mapping:
                    # Check for multiple definitions
                    if node.bid in seen_bids:
                        logger.warning(
                            f"Bid {node.bid} defined in multiple summary entries; overwriting previous value."
                        )
                        multiple_definitions.append(node.bid)
                    seen_bids[node.bid] = mapping[node.bid]
                    node.summary = mapping[node.bid]
                    # Recurse children and filter them
                    if node.children:
                        node.children = recurse(node.children)
                    filtered_nodes.append(node)
                else:
                    # Node is removed if summary is not provided.
                    logger.debug(f"Removing node with bid {node.bid} due to missing summary.")
                # End loop of current nodes.
