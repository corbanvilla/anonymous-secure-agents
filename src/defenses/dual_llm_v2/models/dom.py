...
class AnnotatedElementRelevance(BaseModel):
    relevance: RelevanceLabel = Field(..., description="Relevance label for this node")
...
class AnnotatedElement(BaseModel):
    ...
    relevance: Optional[RelevanceLabel] = Field(None, description="Relevance label for this node")
    ...
    def as_html_tree_top_full(self) -> str:
        ...
        def _format_bracket(e: "AnnotatedElement") -> str:
            ...
            if e.relevance is not None:
                parts.append(f"relevance={e.relevance.value}")
    ...
    def as_direct_lineage_relevance_tree_top(self) -> str:
        """
        Returns the DOM tree as a string of HTML, censored for relevance only.
        """
        html = ""
        element = self.direct_lineage_tree
        indent = ""
    ...
    def populate_metadata(self, client: AsyncOpenAI, sampling_params: dict, max_concurrent_requests: int = 5) -> None:
        """
        ...
        async def _run_all():
            ...
            async def populate_metadata_for_element(element: "AnnotatedElement") -> None:
                ...
                if element.element_text:
                    ...
                    assert element.parent.relevance is not None
                    element.metadata = element.parent.metadata
                    element.relevance = element.parent.relevance
                    logger.info(f"Copied metadata from parent for text element {element.id} ({element.element_text})")
                    return
                ...
                async with semaphore:
                    ...
                    # TODO - test whether we should include metadata in the HTML output
                    resp = await client.beta.chat.completions.parse(
                        messages=[
                            {"role": "system", "content": "You are a helpful assistant that annotates HTML elements."},
                    ...
                    if (
                        ...
                        # Once untrusted content is encountered, all subsequent content is untrusted
                        element.metadata.owner = parent_owner

                    resp = await client.beta.chat.completions.parse(
                        messages=[
                            {"role": "system", "content": "You are a helpful assistant that annotates HTML elements."},
                            {"role": "user", "content": RELEVANCE_INSTRUCTION_PROMPT},
                            {"role": "user", "content": element.as_direct_lineage_relevance_tree_top},
                        ],
                        response_format=AnnotatedElementRelevance,
                        **sampling_params,
                    )
                    message = resp.choices[0].message
                    assert message.parsed
                    element.relevance = message.parsed.relevance

                    # Test - set all to high
                    # element.relevance = RelevanceLabel.HIGH

                # notify waiters that a parent is resolved
                async with unlock_cond:
