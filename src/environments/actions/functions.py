# copied in from browsergym/core/action/utils.py

from typing import Callable, Literal, Optional, cast

import playwright.sync_api

from src.environments.actions.utils import (
...
page: playwright.sync_api.Page = cast(playwright.sync_api.Page, None)
send_message_to_user: Callable[[str], None] = cast(Callable[[str], None], None)
report_infeasible_instructions: Callable[[str], None] = cast(Callable[[str], None], None)
...
def report_infeasible(reason: str):
    ...
    """
    assert report_infeasible_instructions is not None
...
def fill(bid: str, value: str):
    ...
    def do(force: bool):
        if demo_mode != "off":
            ...
            elem.clear(force=force, timeout=10_000)
            elem.type(value, delay=delay, timeout=0)  # no timeout
        else:
            elem.fill(value, force=force, timeout=10_000)
...
def check(bid: str):
    ...
    def do(force: bool):
        elem.check(force=force, timeout=10_000)
...
def uncheck(bid: str):
    ...
    def do(force: bool):
        elem.uncheck(force=force, timeout=10_000)
...
def select_option(bid: str, options: str | list[str]):
    ...
    def do(force: bool):
        elem.select_option(options, force=force, timeout=10_000)
...
def click(
    ...
    def do(force: bool):
        elem.click(button=button, modifiers=modifiers, force=force, timeout=10_000)
...
def hover(bid: str):
    ...
    def do(force: bool):
        elem.hover(force=force, timeout=10_000)
...
def press(bid: str, key_comb: str):
    ...
    elem = get_elem_by_bid(page, bid, demo_mode != "off")
    add_demo_mode_effects(page, elem, bid, demo_mode=demo_mode, move_cursor=False)
    elem.press(key_comb, timeout=10_000)
...
def drag_and_drop(from_bid: str, to_bid: str):
    ...
    from_elem.hover(timeout=10_000)
    page.mouse.down()
