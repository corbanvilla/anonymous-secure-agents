...
from src.agents.occam.processor import TextObervationProcessor, TreeNode
...
def action_remove_redundant_statictext_node(node: TreeNode):
    ...
    if (
        (not node.name)
        or (node.parent and node.name in node.parent.name)
        or (node.parent and any(node.name in sibling.name for sibling in node.siblings()))
    ):
        node.visible = False
...
def action_merge_menuitem_and_option(node: TreeNode):
    if not node.visible:
        return
    if not (
        (node.visible_children() and all(c.role == "menuitem" for c in node.visible_children()))
        or (node.visible_children() and all(c.role == "option" for c in node.visible_children()))
    ):
        return
    if node.visible_children()[0].role == "menuitem":
        if not node.name.strip():
            node.name = "; ".join(
                [action_return_visible_node(c).strip()[len("menuitem ") :] for c in node.visible_children()]
            )
        else:
            node.name += ": " + "; ".join(
                [action_return_visible_node(c).strip()[len("menuitem ") :] for c in node.visible_children()]
        ...
        if not node.name.strip():
            node.name = "; ".join(
                [action_return_visible_node(c).strip()[len("option ") :] for c in node.visible_children()]
            )
        else:
            node.name += ": " + "; ".join(
                [action_return_visible_node(c).strip()[len("option ") :] for c in node.visible_children()]
...
def action_merge_description_list(node: TreeNode):
    ...
    list_term_buffer: list[TreeNode] = []
    for child in node.visible_children():
...
def action_reformat_table(node: TreeNode):
    ...
    def merge_gridcell(gridcell_node: TreeNode):
        if (
            gridcell_node.role not in ["gridcell", "columnheader", "rowheader", "LayoutTableCell"]
            or not gridcell_node.visible
        ):
            return
        gridcell_buffer: list[str] = []
    ...
    try:
        if node.role == "table":
            ...
            def reformat_subtable(row_list, current_table_children):
                # Avoid expensive deep copies when reformatting large tables.
                # A shallow copy is sufficient because new rows are created as
                # new ``TreeNode`` instances and existing nodes are not
                # mutated.
                new_table_children = list(current_table_children)
                if row_list:
                    ...
                    if any(row_0_child.role == "columnheader" for row_0_child in row_list[0].children):
                        if new_table_children and any(n.visible for n in new_table_children):
                            new_table_children.append(
                                TreeNode(node_id=row_list[0].node_id, role="row", name="", depth=row_list[0].depth)
                            )
                        ...
                        new_table_children.append(
                            TreeNode(
                                node_id=row_list[0].node_id,
                                role="row",
                                name="| " + " | ".join(titles) + " |",
                                depth=row_list[0].depth,
                            )
                        )
                        new_table_children.append(
                            TreeNode(
                        ...
                        for row in row_list:
                            new_table_children.append(
                                TreeNode(
                                    node_id=row.node_id,
                                    role="row",
                                    name="| " + " | ".join([row_element.name for row_element in row.children]) + " |",
                                    depth=row.depth,
                                )
                            )
                    else:
            ...
            new_table_children: list[TreeNode] = []
            row_list = []
            row_mode = False
            for child in node.children:
                if child.role == "row":
                ...
                if child.role == "row" and any(row_child.role == "columnheader" for row_child in child.children):
                    ...
                    if row_mode:
                        ...
                        new_table_children = reformat_subtable(
                            row_list=row_list, current_table_children=new_table_children
                        )
                        row_list = [child]
                    row_mode = True
            ...
            def merge_adjacent_text_nodes(nodes):
                ...
                for i in range(1, len(nodes)):
                    if (
                        current_node.visible
                        and current_node.role
                        in ["LayoutTableCell", "StaticText", "generic"] + list(set(ROLE_REPLACEMENT_DICT.values()))
                        and nodes[i].visible
                        and nodes[i].role
                        in ["LayoutTableCell", "StaticText", "generic"] + list(set(ROLE_REPLACEMENT_DICT.values()))
                    ):
            ...
            def dfs_merge_text(n: TreeNode):
                ...
                if (
                    len(n.visible_children()) == 1
                    and n.visible_children()[0].role
                    in ["LayoutTableCell", "StaticText", "generic"] + list(set(ROLE_REPLACEMENT_DICT.values()))
                    and n.role
                    in ["LayoutTableCell", "StaticText", "generic"] + list(set(ROLE_REPLACEMENT_DICT.values()))
                ):
...
def translate_node_to_str(node: TreeNode, mode="concise", **kwargs):
    ...
    parse_node_descendants(
        node, partial(action_return_visible_node, intent_bias=node.depth, mode=mode, **kwargs), tree_buffer=tree_buffer
    )
...
def prune_tree(root_node, mode="str", filter_mode="verbose"):
    ...
    parse_node_descendants(root_node_copy, action_remove_redundant_statictext_node)
    parse_node_descendants(
        root_node_copy, partial(action_replace_node_role, role_replacement_dict=ROLE_REPLACEMENT_DICT)
    )
    ...
    if mode == "str":
        browser_content = translate_node_to_str(node=root_node_copy, mode=filter_mode)
    elif mode == "node":
...
def parse_tree_for_filter(axtree_object) -> str:
    _, _, root_node = TextObervationProcessor.parse_accessibility_tree(axtree_object["nodes"])
    return prune_tree(root_node, mode="str", filter_mode="verbose")


def parse_tree_for_agent(axtree_object) -> str:
    _, _, root_node = TextObervationProcessor.parse_accessibility_tree(axtree_object["nodes"])
    return prune_tree(root_node, mode="str", filter_mode="concise")
