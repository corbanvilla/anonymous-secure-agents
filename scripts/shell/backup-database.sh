...
POSTGRES_CONTAINER=$(sudo docker compose ps | grep postgres | cut -d' ' -f1)
...
sudo docker exec -t $POSTGRES_CONTAINER pg_dump -d $POSTGRES_DATABASE -U $POSTGRES_USERNAME | gzip > $BACKUP_FILE.sql.gz
